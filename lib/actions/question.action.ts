  ...
  try {
    ...
    if (tagsToRemove.length > 0) {
      ...
      await TagQuestion.deleteMany(
        { tagId: { $in: tagIdsToRemove }, question: questionId },
        { session }
      );
...
export async function getRecommendedQuestion({
  ...
  // 1.Find interaction with userId
  ...
  // 2.Find an array of question id from interactions []
  const interactedQuestionIds = interactions.map(
    (interaction) => interaction.actionId
  );

  // 3.Find questions from question id []
  const interactionQuestions = await Question.find({
    _id: { $in: interactedQuestionIds },
  }).select("tags");

  // 4.Find tags id from questions []
  const allTags = interactionQuestions.flatMap((question) =>
    question.tags.map((tag: Types.ObjectId) => tag.toString())
  );

  // 5.Make Tag id unique []
  const uniqueTagIds = [...new Set(allTags)];

  // 6.Set up the query options
...
export async function getQuestions(
  ...
  try {
    if (filter === "recommended") {
      ...
      const recommended = await getRecommendedQuestion({
        userId,
        query,
        skip,
        limit,
...
export async function incrementViews(
  ...
  try {
    ...
    return {
      success: true,
      data: { views: JSON.parse(JSON.stringify(question.views)) },
    };
  } catch (error) {
    return handleError(error) as ErrorResponse;
  }
