...
import { BADGES_CRITERIA } from "@/constants";
...
export const assignBadges = (params: {
  criteria: {
    type: keyof typeof BADGES_CRITERIA;
    count: number;
  }[];
}) => {
  ...
  criteria.forEach((item) => {
    ...
    const badgeLevels = BADGES_CRITERIA[type];

    Object.keys(badgeLevels).forEach((level) => {
      if (count >= badgeLevels[level as keyof typeof badgeCounts]) {
...
export function processJobTitle(title: string | null | undefined): string {
